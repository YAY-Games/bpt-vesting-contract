...
const BptVesting2StrategicMock = artifacts.require('BptVesting2StrategicMock');
...
contract('BptVesting2Strategic', function (accounts) {
    ...
    async function claimInAllCases(rewardAfterTge, stepReward, deltaTime, accountPos) {
        ...
        it('before TGE', async function () {
            ...
            await expectRevert(
                this.BptVesting2Strategic.claim(this.balances[accountPos][1], this.balances[accountPos][2], proof, {from: accounts[accountPos]}),
        ...
        it('after TGE', async function () {
            ...
            let result = (await this.BptVesting2Strategic.claim.call(this.balances[accountPos][1], this.balances[accountPos][2], proof, {from: accounts[accountPos]}));
            ...
            expect((await this.BptVesting2Strategic.tgeIsClaimed.call(accounts[accountPos]))).to.equal(false);
            expect((await this.BptVesting2Strategic.lastClaimedStep.call(accounts[accountPos])).toString()).to.equal("0");
            expect((await this.BptVesting2Strategic.alreadyRewarded.call(accounts[accountPos])).toString()).to.equal("0");
            
            let txReceipt = await this.BptVesting2Strategic.claim(this.balances[accountPos][1], this.balances[accountPos][2], proof, {from: accounts[accountPos]});

            expect((await this.BptVesting2Strategic.tgeIsClaimed.call(accounts[accountPos]))).to.equal(true);
            expect((await this.BptVesting2Strategic.lastClaimedStep.call(accounts[accountPos])).toString()).to.equal("0");
            expect((await this.BptVesting2Strategic.alreadyRewarded.call(accounts[accountPos])).toString()).to.equal(expectedReward.toString());
        ...
        it('after TGE + step', async function () {
            ...
            expect((await this.BptVesting2Strategic.tgeIsClaimed.call(accounts[accountPos]))).to.equal(true);
            expect((await this.BptVesting2Strategic.lastClaimedStep.call(accounts[accountPos])).toString()).to.equal("1");
            expect((await this.BptVesting2Strategic.alreadyRewarded.call(accounts[accountPos])).toString()).to.equal(expectedReward.toString());
            ...
            await this.BptVesting2Strategic.claim(this.balances[accountPos][1], this.balances[accountPos][2], proof, {from: accounts[accountPos]});
            await expectRevert(
                this.BptVesting2Strategic.claim(this.balances[accountPos][1], this.balances[accountPos][2], proof, {from: accounts[accountPos]}),
                'BptVesting: no tokens to claim',
            );
    ...
    describe('constructor', function () {
        ...
        beforeEach(async function () {
            ...
            this.BptVesting2Strategic = await BptVesting2Strategic.new(
                this.token.address,
                this.merkleRoot,
                this.tgeTimestamp
            );
        ...
        it('positive', async function () {
            expect(await this.BptVesting2Strategic.token.call()).to.equal(this.token.address);
            expect(await this.BptVesting2Strategic.mercleRoot.call()).to.equal(this.merkleRoot);
            expect((await this.BptVesting2Strategic.tgeTimestamp.call()).toNumber()).to.equal(this.tgeTimestamp);
            
        });

        it('negative', async function () {
            await expectRevert(
                BptVesting2Strategic.new(
                    this.token.address,
                    this.merkleRoot,
                    this.tgeTimestamp - 10
                ),
                "BptVesting2Strategic: wrong TGE timestamp",
            );
            await expectRevert(
                BptVesting2Strategic.new(
                    ZERO_ADDRESS,
                ...
                "BptVesting2Strategic: zero token address",
            );
            await expectRevert(
                BptVesting2Strategic.new(
                    this.token.address,
                ...
                "BptVesting2Strategic: zero mercle root",
            );
        });
    ...
    describe('mock contract', function () {
        beforeEach(async function () {
            ...
            this.BptVesting2Strategic = await BptVesting2StrategicMock.new(
                this.token.address,
                this.merkleRoot,
                this.tgeTimestamp,
                STEP_COUNT
            ...
            await this.token.transfer(this.BptVesting2Strategic.address, this.totalTokens);
        });
    ...
    describe('real contract', function () {
        ...
        describe('verify', function () {
            it('positive proof', async function () {
                ...
                const result1 = await this.BptVesting2Strategic.checkClaim(this.balances[0][0], this.balances[0][1], this.balances[0][2], proof1)
                expect(result1).to.equal(true);
    
                const proof2 = this.merkleTree.getHexProof(this.elems[1]);
                const result2 = await this.BptVesting2Strategic.checkClaim(this.balances[1][0], this.balances[1][1], this.balances[1][2], proof2)
                expect(result2).to.equal(true);
    
                const proof3 = this.merkleTree.getHexProof(this.elems[2]);
                const result3 = await this.BptVesting2Strategic.checkClaim(this.balances[2][0], this.balances[2][1], this.balances[2][2], proof3)
                expect(result3).to.equal(true);
    
                const proof4 = this.merkleTree.getHexProof(this.elems[3]);
                const result4 = await this.BptVesting2Strategic.checkClaim(this.balances[3][0], this.balances[3][1], this.balances[3][2], proof4)
                expect(result4).to.equal(true);
            });
            it('negative proof', async function () {
                const proof1 = this.merkleTree.getHexProof(this.elems[0]);
                const result1 = await this.BptVesting2Strategic.checkClaim(this.balances[1][0], this.balances[0][1], this.balances[0][2], proof1)
                expect(result1).to.equal(false);
    
                const proof2 = this.merkleTree.getHexProof(this.elems[1]);
                const result2 = await this.BptVesting2Strategic.checkClaim(this.balances[0][0], this.balances[1][1], this.balances[1][2], proof2)
        ...
        describe('claim', function () {
            it('negative cases', async function () {
                const proof1 = this.merkleTree.getHexProof(this.elems[0]);
                await expectRevert(
                    this.BptVesting2Strategic.claim(this.balances[1][1], this.balances[1][2], proof1, {from: accounts[0]}),
                    'BptVesting: invalid proof or wrong data',
                );
                await expectRevert(
                    this.BptVesting2Strategic.claim(this.balances[0][1], this.balances[0][2], proof1, {from: accounts[1]}),
                    'BptVesting: invalid proof or wrong data',
                );
                await expectRevert(
                    this.BptVesting2Strategic.claim(this.balances[0][1], this.balances[0][2], proof1, {from: accounts[0]}),
                    'BptVesting: TGE has not started yet',
                );

                const proof2 = this.merkleTree.getHexProof(this.elems[4]);
                await expectRevert(
                    this.BptVesting2Strategic.claim(this.balances[4][1], this.balances[4][2], proof2, {from: accounts[4]}),
                    'BptVesting: invalid category',
                );

                const proof3 = this.merkleTree.getHexProof(this.elems[5]);
                await expectRevert(
                    this.BptVesting2Strategic.claim(this.balances[5][1], this.balances[5][2], proof3, {from: accounts[5]}),
            ...
            it('after deadline', async function () {
                ...
                let result = (await this.BptVesting2Strategic.claim.call(this.balances[0][1], this.balances[0][2], proof, {from: accounts[0]}));
                ...
                expect((await this.BptVesting2Strategic.tgeIsClaimed.call(accounts[0]))).to.equal(false);
                expect((await this.BptVesting2Strategic.lastClaimedStep.call(accounts[0])).toString()).to.equal("0");
                expect((await this.BptVesting2Strategic.alreadyRewarded.call(accounts[0])).toString()).to.equal("0");
                
                let txReceipt = await this.BptVesting2Strategic.claim(this.balances[0][1], this.balances[0][2], proof, {from: accounts[0]});
    
                expect((await this.BptVesting2Strategic.tgeIsClaimed.call(accounts[0]))).to.equal(true);
                expect((await this.BptVesting2Strategic.lastClaimedStep.call(accounts[0])).toString()).to.equal("80");
                expect((await this.BptVesting2Strategic.alreadyRewarded.call(accounts[0])).toString()).to.equal(expectedReward.toString());
                ...
                await expectRevert(
                    this.BptVesting2Strategic.claim(this.balances[0][1], this.balances[0][2], proof, {from: accounts[0]}),
                    'BptVesting: no tokens to claim',
                );
            });
        });
